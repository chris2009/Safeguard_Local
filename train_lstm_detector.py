...
╔══════════════════════════════════════════════════════════════════════════════╗
║                                                                              ║
║   🛡️  SAFEGUARD VISION AI - ENTRENAMIENTO LSTM                              ║
║                                                                              ║
║   Entrena modelo LSTM para detectar TRANSICIONES de caída                   ║
║                                                                              ║
...
¿POR QUÉ LSTM?
==============
    Random Forest: "¿Esta POSE parece caída?" → Confunde acostado con caído
    
    LSTM: "¿Hubo MOVIMIENTO de caída en esta secuencia?" → Detecta transiciones
...
=============
    ...
    Dense (32 units) + Dropout
           ↓
    Dense (1 unit, sigmoid) → Probabilidad de caída
...
# TensorFlow/Keras
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'  # Reduce logs de TF
...
OUTPUT_FOLDER = r"D:\APRENDIZAJE\MAESTRIA\CICLO_III\MISTI\Project_local\safeguard_model_lstm"
...
MODEL_CONFIG = {
    # Arquitectura
    "lstm_units_1": 128,        # Unidades en primera capa LSTM
    "lstm_units_2": 64,         # Unidades en segunda capa LSTM
    ...
    # Entrenamiento
    "batch_size": 32,
    "epochs": 100,              # Máximo de épocas (early stopping cortará antes)
    "learning_rate": 0.001,
...
def evaluate_model(model, X_test, y_test, output_folder):
    ...
    for thresh in np.arange(0.1, 0.9, 0.05):
        ...
        recall = sum((y_pred_temp == 1) & (y_test == 1)) / sum(y_test)
    ...
    metrics = {
        ...
        "threshold_optimized": {
            ...
            "precision": float(precision_score(y_test, (y_prob >= best_threshold).astype(int))),
            "recall": float(recall_score(y_test, (y_prob >= best_threshold).astype(int))),
            "f1": float(f1_score(y_test, (y_prob >= best_threshold).astype(int)))
        }
    }
...
def plot_training_history(history, metrics, y_test, y_prob, output_folder):
    ...
    fig.suptitle('🛡️ SafeGuard Vision AI - Entrenamiento LSTM\nDetección de Caídas Temporal', 
...
def main():
    print("\n")
    print("╔" + "═"*70 + "╗")
    print("║" + " 🛡️  SAFEGUARD VISION AI - ENTRENAMIENTO LSTM ".center(70) + "║")
    ...
    print("║     python demo_video_lstm.py".ljust(71) + "║")
    print("╚" + "═"*70 + "╝")
    print()
